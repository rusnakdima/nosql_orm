...
use nosql_orm::sql::{
  SqlColumnDef, SqlColumnType, SqlDialect, SqlIndexDef, SqlIndexType, SqlTableDef,
};
...
fn test_update_sql() {
  let builder = SqlQueryBuilder::new(SqlDialect::SQLite);
  let sql = builder.update_sql("users", &serde_json::json!({"name": "Bob"}), "id", "1");
...
fn test_sql_column_type_to_sql() {
  ...
  assert_eq!(
    SqlColumnType::VarChar(100).to_sql(SqlDialect::MySQL),
    "VARCHAR(100)"
  );
}
...
fn test_foreign_key_builder() {
  let fk = SqlForeignKey::new(vec!["user_id".to_string()], "users", vec!["id".to_string()]);
...
fn test_all_sql_dialects_quote() {
  ...
  assert_eq!(
    SqlDialect::PostgreSQL.quote_identifier(name),
    "\"test_table\""
  );
...
fn test_query_builder_like() {
  use nosql_orm::query::QueryBuilder;

  let qb = QueryBuilder::new()
    .where_like("name", "%admin%")
    .where_gte("age", serde_json::json!(18))
    .limit(10);
...
fn test_filter_matches_like() {
  ...
  let filter = Filter::Like("name".to_string(), "%test%".to_string());
  let doc = serde_json::json!({ "name": "my test value" });
...
#[test]
fn test_filter_matches_is_not_null() {
  use nosql_orm::query::Filter;

  let filter = Filter::IsNotNull("email".to_string());
  assert!(filter.matches(&serde_json::json!({ "email": "test@example.com" })));
