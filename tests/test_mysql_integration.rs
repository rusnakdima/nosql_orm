...
mod mysql_integration_tests {
  ...
  async fn setup_provider() -> OrmResult<MySqlProvider> {
    MySqlProvider::connect(&get_connection_string()).await
  }

  async fn cleanup_users(repo: &Repository<TestUser, MySqlProvider>) -> OrmResult<()> {
