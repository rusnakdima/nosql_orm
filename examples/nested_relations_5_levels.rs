...
async fn main() -> OrmResult<()> {
  ...
  println!(
    "  Level 5: Manager: {} ({})",
    manager.manager_name, manager.email
  );
  println!(
    "  Level 4: Department: {} (manager: {})",
    department.department_name, department.manager_id
  );
  println!(
    "  Level 3: Company: {} (department: {})",
    company.company_name, company.department_id
  );
  println!(
    "  Level 2: Profile: {} (company: {})",
    profile.bio, profile.company_id
  );
  println!(
    "  Level 1: User: {} (profile: {})",
    user.user_name, user.profile_id
  );
  ...
  if let Some(author) = post_with_author.one("author")? {
    println!(
      "  Author: {} (id: {})",
      author.get("user_name").unwrap(),
      author.get("id").unwrap()
    );
  }
  ...
  if let Some(mgr) = dept_with_manager.one("manager")? {
    println!(
      "  Manager: {} ({})",
      mgr.get("manager_name").unwrap(),
      mgr.get("email").unwrap()
    );
  }
