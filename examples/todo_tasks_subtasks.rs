...
async fn main() -> OrmResult<()> {
  ...
  let todo = todos_repo
    .save(Todo {
      ...
      task_ids: vec![
        task1.id.clone().unwrap(),
        task2.id.clone().unwrap(),
        task3.id.clone().unwrap(),
      ],
      deleted_at: None,
    })
    .await?;
  ...
  for task_val in tasks_slice {
    let task_title = task_val
      ...
      .and_then(|v| v.as_str())
      .unwrap_or("?");
    let task_status = task_val
      .get("task_status")
      .and_then(|v| v.as_str())
      .unwrap_or("?");
    let subtask_ids_count = task_val
      .get("subtask_ids")
    ...
    println!(
      "    - '{}' (status: {}, subtasks: {})",
      task_title, task_status, subtask_ids_count
    );
  }
  ...
  for task_val in tasks_slice {
    ...
    for subtask_val in subtasks_slice {
      let title = subtask_val
        .get("subtask_title")
        .and_then(|v| v.as_str())
        .unwrap_or("?");
      let status = subtask_val
        .get("subtask_status")
        .and_then(|v| v.as_str())
        .unwrap_or("?");
      let priority = subtask_val
        .get("subtask_priority")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
      println!(
        "    - '{}' (status: {}, priority: {})",
