...
impl TodoFullLoader {
  ...
  async fn load_subtasks_for_task(&mut self, subtask_ids: &[String]) -> OrmResult<Vec<Subtask>> {
    ...
    for sid in subtask_ids {
      let sub_val = if let Some(subtask) = self.subtasks_cache.get(sid).cloned() {
        ...
        let sub: Subtask =
          serde_json::from_value(found.clone()).map_err(OrmError::Serialization)?;
  ...
  async fn load_tasks_for_todo(
    &mut self,
    task_ids: &[String],
  ) -> OrmResult<Vec<(Task, Vec<Subtask>)>> {
    let mut result = Vec::new();
    for tid in task_ids {
  ...
  async fn load_todo_with_everything(
    &mut self,
    todo_id: &str,
  ) -> OrmResult<Option<(Todo, Vec<(Task, Vec<Subtask>)>)>> {
...
fn print_todo_full(todo: &Todo, tasks_with_subtasks: &[(Task, Vec<Subtask>)]) {
  let output = TodoWithAllRelations {
    ...
    tasks: tasks_with_subtasks
      .iter()
      .map(|(task, subtasks)| TaskWithSubtasks {
  ...
  for (i, (task, subtasks)) in tasks_with_subtasks.iter().enumerate() {
    ...
    if subtasks.is_empty() {
      ...
      for subtask in subtasks {
        let check = if subtask.subtask_status == "completed" {
          "✓"
        } else {
          "○"
        };
        println!(
          "  │       {} [{}] {} - priority: {}",
          check, subtask.subtask_status, subtask.subtask_title, subtask.subtask_priority
        );
      }
    }
  }
  ...
  let completed: usize = tasks_with_subtasks
    .iter()
    .flat_map(|(_, s)| s.iter())
    .filter(|st| st.subtask_status == "completed")
    .count();
  println!(
    "  Total Tasks: {} | Total Subtasks: {} | Completed: {}/{}",
    tasks_with_subtasks.len(),
    total_subtasks,
    completed,
    total_subtasks
...
async fn main() -> OrmResult<()> {
  ...
  for (title, desc, status, subtask_indices) in tasks_data {
    let subtask_ids: Vec<String> = subtask_indices
      .iter()
  ...
  println!(
    "=== LOADING TODO #{} WITH ALL RELATIONS (ONE CALL) ===\n",
    &todo.id.as_ref().unwrap()[..8]
  );

  let mut loader = TodoFullLoader::new(provider.clone());
  if let Some((loaded_todo, tasks_with_subtasks)) = loader
    .load_todo_with_everything(todo.id.as_ref().unwrap())
    .await?
  {
