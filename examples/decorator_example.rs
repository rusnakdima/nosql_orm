...
async fn main() -> OrmResult<()> {
    ...
    let post_relation_repo: RelationRepository<DecoratorPost, _> = RelationRepository::new(provider.clone());
    let category_repo: Repository<DecoratorCategory, _> = Repository::new(provider);
    
    // Create a user
    let user = user_repo.save(DecoratorUser {
        id: None,
        name: "John Doe".into(),
    ...
    // Create a category
    let category = category_repo.save(DecoratorCategory {
        id: None,
        name: "Rust".into(),
    }).await?;
    ...
    let post = post_repo.save(DecoratorPost {
        id: None,
    ...
    println!("  Author: {}", post_with_relations.one("author")?.map(|a| a["name"].as_str().unwrap_or("")).unwrap_or(""));
    println!("  Categories: {:?}", post_with_relations.many("categories")?);
