  ...
  async fn load_recursive(
    ...
    for (rel_name, fk_field, target_table) in relations {
      if let Some(fk_value) = doc.get(fk_field).and_then(|v| v.as_str()) {
        let nested =
          Box::pin(self.load_recursive(target_table, fk_value, depth, current_depth + 1)).await;
...
async fn main() -> OrmResult<()> {
  ...
  let root_with_l1 = repo_root
    .find_with_relations(root.id.as_ref().unwrap(), &["level1"])
    .await?
    .unwrap();
  ...
  if let Some(l1_data) = root_with_l1.one("level1")? {
    ...
    let l1_with_l2 = repo1
      .find_with_relations(l1_id, &["level2"])
      .await?
      .unwrap();
    if let Some(l2_data) = l1_with_l2.one("level2")? {
      ...
      let l2_with_l3 = repo2
        .find_with_relations(l2_id, &["level3"])
        .await?
        .unwrap();
      if let Some(l3_data) = l2_with_l3.one("level3")? {
        ...
        let l3_with_l4 = repo3
          .find_with_relations(l3_id, &["level4"])
          .await?
          .unwrap();
