...
use mongodb::{
    bson::{doc, Document, Bson, to_bson, from_bson},
...
impl MongoProvider {
    ...
    fn json_to_bson(value: Value) -> OrmResult<Document> {
        let bson = to_bson(&value).map_err(|e| OrmError::Serialization(
    ...
    fn bson_to_json(doc: Document) -> OrmResult<Value> {
        ...
        let json: Value = from_bson(bson).map_err(|e| OrmError::Serialization(
            serde_json::Error::custom(e.to_string()),
        ))?;
    ...
    fn filter_to_doc(filter: &Filter) -> Document {
        match filter {
            ...
            Filter::In(f, vals) => {
                let bson_vals: Vec<Bson> = vals.iter()
            ...
            Filter::And(filters) => {
                let docs: Vec<Bson> = filters.iter()
...
impl DatabaseProvider for MongoProvider {
    async fn insert(&self, collection: &str, mut doc: Value) -> OrmResult<Value> {
        if doc.get("id").and_then(|v| v.as_str()).map_or(true, |s| s.is_empty()) {
    ...
    async fn patch(&self, collection: &str, id: &str, patch: Value) -> OrmResult<Value> {
        ...
        coll.update_one(doc! { "_id": id }, doc! { "$set": patch_doc }, None).await?;

        self.find_by_id(collection, id).await?.ok_or_else(|| {
            OrmError::NotFound(format!("{}/{}", collection, id))
        })
    }
