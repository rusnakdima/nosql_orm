...
impl SqliteProvider {
  pub async fn new(database_path: &str) -> OrmResult<Self> {
  ...
  fn value_to_sqlite_param(value: &Value) -> String {
    match value {
  ...
  fn filter_to_where(filter: &Filter) -> (String, Vec<String>) {
    ...
    match filter {
      Filter::Eq(field, value) => {
        ...
        params.push(Self::value_to_sqlite_param(value));
      }
      ...
      Filter::In(field, values) => {
        ...
        for v in values {
          params.push(Self::value_to_sqlite_param(v));
        }
      }
      ...
      Filter::And(filters) => {
        let parts: Vec<String> = filters
          .iter()
          .map(|f| {
            let (c, _) = Self::filter_to_where(f);
            c
          })
          .collect();
      ...
      Filter::Not(inner) => {
        let (cond, _) = Self::filter_to_where(inner);
...
impl DatabaseProvider for SqliteProvider {
  async fn insert(&self, collection: &str, doc: Value) -> OrmResult<Value> {
    ...
    if let Some(obj) = doc_with_id.as_object() {
      for (_, v) in obj {
        query = query.bind(Self::value_to_sqlite_param(v));
      }
    }
  ...
  async fn update(&self, collection: &str, id: &str, doc: Value) -> OrmResult<Value> {
    ...
    if let Some(obj) = doc.as_object() {
      for (key, value) in obj {
        if key != "id" {
          query = query.bind(Self::value_to_sqlite_param(value));
        }
      }
    }
    query = query.bind(id);
