...
fn parse_validate_meta(meta: &syn::Meta) -> Vec<ValidationType> {
    ...
    match meta {
        ...
        syn::Meta::NameValue(nv) => {
            let ident_str = nv.path.get_ident().map(|i| i.to_string()).unwrap_or_default();
...
fn generate_validation_block(field_name: &syn::Ident, validations: &[ValidationType]) -> proc_macro2::TokenStream {
...
fn generate_check(field_name: &syn::Ident, validation: &ValidationType) -> proc_macro2::TokenStream {
    match validation {
