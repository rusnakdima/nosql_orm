...
fn parse_relation_attr(
  ...
  let is_cascade = on_delete_str.contains("Cascade");
  let on_delete = if is_cascade {
  ...
  match rel_type {
    "one_to_many" => {
      let mut rel = quote! {
          ::nosql_orm::relations::RelationDef::one_to_many(#name, #target, #key).on_delete(#on_delete).cascade_hard_delete(#is_cascade).cascade_soft_delete(#is_cascade)
      };
    ...
    "many_to_many" => Some(quote! {
        ::nosql_orm::relations::RelationDef::many_to_many(#name, #target, #key).on_delete(#on_delete).cascade_hard_delete(#is_cascade).cascade_soft_delete(#is_cascade)
    }),
    _ => None,
  }
}
...
fn parse_cascade_attr(ts_str: &str, attr_name: &str) -> Option<bool> {
  if ts_str.contains(attr_name) {
    Some(
      ts_str.contains(&format!("{}(true)", attr_name))
        || ts_str.contains(&format!("{} = true", attr_name)),
    )
  } else {
    None
